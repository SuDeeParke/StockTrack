...
- [x] **T10** · 后端：回测 API（封装 InStock 回测引擎，异步 job）
  - 依赖：Phase 1 完成
- [x] **T11** · 前端：回测界面（策略选择 + 权益曲线 + 统计卡片）
  - 依赖：T10 + T6

### ✅ Phase 2 Checkpoint
- [x] 完整回测流程走通截图
- [x] 回测数值与 InStock 命令行一致
...
- [x] **T12** · 后端：Futu OpenAPI 集成（持仓 / 下单 / 风控）
  - 依赖：Phase 1 完成
- [x] **T13** · 前端：交易执行面板（持仓总览 + 信号下单流程）
  - 依赖：T12 + T6

### ✅ Phase 3 Checkpoint
- [x] Futu 模拟账户完成 3 笔完整下单流程
- [x] 风控拦截所有边界情况
